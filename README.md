...
It allocates only when utilization backoff permits and, by default, concurrently
drives Ascend AI Core/Cube, AI Vector, and HBM bandwidth. It releases cleanly on
exit.
...
keep-npu --npu-ids 4,5,6,7 --vram 1GiB --interval 60 --busy-threshold -1
```
...
The default `mixed` workload runs FP16 matrix multiplication and Vector
operations concurrently on independent streams. Its single `--vram` value is a
per-NPU capacity budget shared by Cube matrices and Vector buffers; it does not
mean an HBM bandwidth percentage. Continuous operations over those buffers are
what create HBM bandwidth pressure. Use `--workload aicore` for Cube-only
